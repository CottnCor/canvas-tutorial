...
export const Stats: () => IStats = function () {
    let mode = 0;
    ...
    function addPanel(panel: IPanel) {
    ...
    let frames = 0;
    let memPanel = {} as IPanel;
    const fpsPanel = addPanel(Panel('FPS', '#0ff', '#002'));
    const msPanel = addPanel(Panel('MS', '#0f0', '#020'));
    // if (window.performance && window.performance.memory) {
    //     memPanel = addPanel(Panel('MB', '#f08', '#201'));
    // }
    showPanel(0);
    return {
        REVISION: 16,
...
export const Panel: (name: string, fg: string, bg: string) => IPanel = function (name, fg, bg) {
    let min = Infinity;
    let max = 0;
